...
import { ChatResponse, Product } from "@/lib/interface";
import { categories, sofaTypes, subcategories } from "@/lib/data";
...
const AdminDashboard = () => {
    ...
    if (activeCategory !== "all") {
      ...
      if (activeSubcategory) {
        ...
        if (activeSubcategory === "sofas" && activeSofaType) {
          filtered = filtered.filter(
            (product) => product.sofaType === activeSofaType
          );
        }
      }
    }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fallback to localStorage if axios fails
  ...
  const handleProductSave = async () => {
    ...
    try {
      ...
      localStorage.setItem(
        "furnitureProducts",
        JSON.stringify([
          ...products.filter((p) => p._id !== savedProduct._id),
          savedProduct,
        ])
      );
  ...
  const handleResponseSave = async () => {
    ...
    setLoading(true);
    try {
      let savedResponse: ChatResponse;
      ...
      localStorage.setItem(
        "chatResponses",
        JSON.stringify([
          ...chatResponses.filter((r) => r._id !== savedResponse._id),
          savedResponse,
        ])
      );
  ...
  const handleDeleteProduct = async (id: string) => {
    ...
    try {
      ...
      localStorage.setItem(
        "furnitureProducts",
        JSON.stringify(products.filter((product) => product._id !== id))
      );
  ...
  const handleDeleteResponse = async (id: string) => {
    ...
    try {
      ...
      localStorage.setItem(
        "chatResponses",
        JSON.stringify(chatResponses.filter((response) => response._id !== id))
      );
                  ...
                  <div className="flex space-x-2 pb-2">
                    ...
                    {sofaTypes.map((type) => (
                      <Button
                        key={type.value}
                        variant={
                          activeSofaType === type.value ? "default" : "outline"
                        }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2">
                <Card className="shadow-md">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Manage predefined chat responses
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
              ...
              {currentResponse && (
                <Card className="shadow-md">
                  <CardHeader>
                    <CardTitle>
                      {currentResponse._id ? "Edit Response" : "Add Response"}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
